function sign_message(message:bigint, private_key:bigint): SignatureType{
    ...
    var s: bigint = modMul(modInv(k,ORDER), modAdd(message, modMul(r, private_key, ORDER), ORDER),ORDER);
    return { r: r, s: s } as SignatureType;
}

function verify_sig(signature: SignatureType, message: bigint, public_key:AffinePoint){
...
function getRandomBigInt(n: bigint): bigint { //NOT a cryptographically secure PRNG
    //generates random 53-bit number
    const randomValue = () => BigInt(Math   .floor(Math.random() * Number.MAX_SAFE_INTEGER)).toString(2);
    var randomBigInt: string = "";
    for (var i=0; i<5;i++){ //Goes up to 265-bit numbers (choosen to exceed PRIME)
